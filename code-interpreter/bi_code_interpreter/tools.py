...
def generate_python_code(
    ...
    if data_context:
        try:
            context_dict = json.loads(data_context) if isinstance(data_context, str) else data_context
...
def analyze_results(
    ...
    这个工具会调用LLM对代码执行结果进行深入分析，生成包含业务洞察的报告。
    
    Args:
        original_task: 用户的原始任务描述，例如："分析销售数据的趋势"
        executed_code: 已执行的Python代码
        execution_output: 代码执行的输出结果（stdout）
